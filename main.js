...
const form = document.querySelector("#lorem-input")
...
form.addEventListener("submit", function(e){
    ...
    if(isNaN(value) || value <= 0 || value > 9){
        article.innerHTML = `<p>${text[random]}</p>`
    }
    else{
        let paragraph = text.slice(0,value)
        paragraph = paragraph.map(function(item){
            return `<p>${item}</p>`
        }).join("")
        article.innerHTML = paragraph
    }
})
